...
BEGIN
  ...
  fmt.print (dll_name, "xd_t_%3.3s", opt.RemoteTransport);
  dll_hmod := dll.GetModuleHandle (dll_name);
  IF NOT Dll.CheckHandle(dll_hmod) THEN
    dll_hmod := dll.LoadModule (dll_name);
    IF dll_hmod = NIL THEN
      RETURN msg.DLL_transport_not_found;
    END;
    Dll.AddHandle(dll_hmod);
  END;
  proc := dll.GetProcAdr (dll_hmod, 'InitTransport');
  IF proc = NIL THEN
    RETURN msg.Wrong_format_DLL_transport;
  END;
  init := rmt.TRN_INIT (proc);
  COPY (opt.RemoteTransport, Transport.id);
  xs.Uppercase (Transport.id);
  IF NOT init (Transport) THEN
    Transport.id := '';
...
BEGIN
  ASSERT(CacheMax = 0);
  CacheMax  := SIZE(desc)+sys.CAST(CARDINAL,desc.len);
  CachePos  := 0;
  CacheSend := 0;
