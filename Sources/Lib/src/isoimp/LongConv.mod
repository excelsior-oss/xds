...
<*+ m2extensions *>
IMPLEMENTATION MODULE LongConv; (* Andrew Cadach Aug 1993 *)

(* low-level float/string conversions *)

IMPORT LongStr, ConvTypes;
...
BEGIN
  ...
  <* ELSE *>
    XRaise.RAISE(XRaise.RealConv,"RealConv.Exception");
  <* END *>
END raise;
...
BEGIN
  LongStr.StrToReal (str, real, res);
  RETURN (res);
END FormatReal;
