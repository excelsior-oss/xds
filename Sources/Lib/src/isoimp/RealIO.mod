...
IMPLEMENTATION MODULE RealIO; (* Andrew Cadach Aug 1993 *)

IMPORT IOChan, TextIO, IOConsts, RealStr, CharClass;
IMPORT ConvTypes, RealConv;
...
BEGIN
  ...
  ELSE
    str[n]:=0C;
    RealStr.StrToReal(str,real,conv);
    CASE conv OF
...
BEGIN
  IF ABS(real)>1.0 THEN
    IF RealConv.LengthFixedReal(real,sigFigs)<=width THEN
  ...
  ELSE
    ...
    RealStr.RealToFixed (real, w-2, fix);
    RealStr.RealToFloat (real, w-2, str);
