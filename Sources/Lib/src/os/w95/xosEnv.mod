...
(*
   implementation of environment access functions for Windows95/NT
*)
<*+ M2EXTENSIONS *>
IMPLEMENTATION MODULE xosEnv; (* Hady. 31.05.96 16:27 *)

IMPORT  SYSTEM, xmRTS, xWin32, X2C;

PROCEDURE ["C"] X2C_EnvStringLength(name: xmRTS.X2C_pCHAR): SYSTEM.CARD32;
BEGIN
  RETURN xWin32.GetEnvironmentVariableA(name,NIL,0);
END X2C_EnvStringLength;

PROCEDURE ["C"] X2C_EnvString(name: xmRTS.X2C_pCHAR; buf: xmRTS.X2C_pCHAR; blen: SYSTEM.CARD32);
BEGIN
  IF (blen=0) OR (buf=NIL) THEN RETURN END;
  IF xWin32.GetEnvironmentVariableA(name,buf,blen)=0 THEN buf^:=0C END;
END X2C_EnvString;
