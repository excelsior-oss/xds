...
\TRef{PROJECT mode}{xc:modes:project} (see below).
...
In the \TRef{PROJECT mode}{xc:modes:project}, if the \OERef{DLLNAME} equation was
...
\verb'    xlink . . . /dll=mydll.edf'
      ...
      \TRef{XDS Library Manager}{xlib} to produce it
...
\item If the \OERef{IMPLIB} option is set ON, the compiler expects that
      ...
      will be available at link time. You have to use \TRef{XLIB}{xlib}
      ...
      \TRef{{\tt !module} directive}{xc:project}, provided that
      the \TRef{template file}{xc:template} has appropriate support.
