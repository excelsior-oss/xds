...
In the latter case, a \TRef{pop-up menu}{dialog:intro:menus} will also be displayed.
...
a step command is executed or a \TRef{breakpoint}{dialog:breaks} is hit.
...
the current \TRef{component}{dialog:navigating:components}.
...
\item[{\bf Run to cursor}] \mbox{} \\
    Execute until the current \TRef{user cursor}{dialog:navigating:cursors}
    position is reached.
...
this command if you accidentally issued the \TRef{{\bf Step into}}
{dialog:executing:stepinto} command instead of
\TRef{{\bf Step over}}{dialog:executing:stepover}.
...
\TRef{executable line}{start:general:executable},
...
\TRef{breakpoints}{dialog:breaks}, \TRef{breaks}{dialog:breaks},
and \TRef{watches}{dialog:data:watches} are retained.
...
unlike a \TRef{simple breakpoint}{dialog:breaks:simple}.
...
\TRef{sticky breakpoint}{dialog:breaks:sticky}
...
When the \TRef{user cursor}{dialog:navigating:cursors}
...
When the \TRef{user cursor}{dialog:navigating:cursors} is positioned over
...
The effect is that all \TRef{information windows}{dialog:intro}
...
the \TRef{user cursor}{dialog:navigating:cursors} to the
...
the {\bf Type} \TRef{pop-up submenu}{dialog:intro:menus}. You may change radix
...
\TRef{{\bf Modules} window}{dialog:navigating:modules}.
...
\TRef{{\bf Call Stack}}{dialog:executing:callstack} window.
...
\item[{\bf Code highlight}] \mbox{} \\
    If enabled, XD displays \TRef{executable lines}{start:general:executable}
    in different color.
...
% !!! New in version 1.1:
    ...
    \TRef{pop-up menu}{dialog:intro:menus}.
...
\TRef{session file}{dialog:sysfiles}
