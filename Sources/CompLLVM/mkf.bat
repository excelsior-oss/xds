...
rem   Release   - compile in release mode
...
if not exist "src"  mklink /D /J "src"  "..\Comp\src" || exit /B 1
...
if "%MAKE_MODE_RELEASE%" == "yes" set MAKE_OPTIONS=%MAKE_OPTIONS% -mode:=release 

echo %~dp0
echo xc =p %MAKE_TASK%.prj  %MAKE_OPTIONS%
xc =p %MAKE_TASK%.prj  %MAKE_OPTIONS%
...
if "%IS_COMPONENT%" == "yes" (
    ...
    echo   Release   - make in release mode
    exit /B 1
)
shift /1
